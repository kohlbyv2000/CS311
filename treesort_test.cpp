...
#include "treesort.hpp"      // For treesort
#include "treesort.hpp"      // Double-inclusion check, for testing only
...
// If static member _compThrow is set, then operator< throws
//  std::runtime_error. Exception object's "what" member is set to "<".
...
// Invariants:
//     MovableCounter::_existing is number of existing objects of this
//      class.
//     MovableCounter::_ctorCount is number of successful ctor calls
//      since most recent call to reset, or start of program if reset
//      has never been called.
//     MovableCounter::_dctorCount is (similarly) number of dctor calls.
//     MovableCounter::_assnCount is (similarly) number of copy assn
//      calls.
//     MovableCounter::_maxExisting is (similarly) highest value
//      _existing has assumed.
class MovableCounter {
...
public:
    ...
    // May throw std::runtime_error
    // Strong Guarantee
    MovableCounter & operator=(
        [[maybe_unused]] const MovableCounter & rhs)
    {
        if (_copyThrow)
...
private:
    ...
    static size_t _ctorCount;    // # of successful (non-throwing) ctor
                                 //  calls
    static size_t _dctorCount;   // # of dctor calls
    static size_t _assnCount;    // # of successful (non-throwing)
                                 //  copy = calls
    static bool _copyThrow;      // true if copy operations (ctor, =)
                                 //  throw
...
// Dummy-ish operator<, forming a strict weak order for MovableCounter
//  class
...
TEST_CASE( "treesort: small lists of int" )
{
    ...
    SUBCASE( "std::list<int>" )
    {
        ...
        auto lit2 = lit1;
        treesort(lit1, lit2);
        REQUIRE( equal(data_l.begin(), data_l.end(),
                       dataCheck.begin()) );
        }
...
TEST_CASE( "treesort: object counts" )
{
    SUBCASE( "Count additional value-type objects used in sort" )
    {
        ...
        size_t add_objs = MovableCounter::getMaxExisting()
                          - base_existing;
