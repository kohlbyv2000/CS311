...
#include "msarray.hpp"       // For class template MSArray
#include "msarray.hpp"       // Double-inclusion check, for testing only
...
TEST_CASE( "Function return types are correct" )
{
    ...
    SUBCASE( "Move assn returns MSArray by reference" )
    {
        ...
        [[maybe_unused]] MSArray<int> & r(mi1 = std::move(mi_movable1));
        REQUIRE( TypeCheck<MSArray<int>>::check(
                   mi1 = std::move(mi_movable2)) );
    }
    ...
    SUBCASE( "Bracket op (const) returns item by ref-to-const" )
    {
        ...
        REQUIRE( TypeCheck<MSArray<double>::value_type>::check(
                   cmd5[0]) );
    }
    ...
    SUBCASE( "size returns size_type by value" )
    {
        ...
        REQUIRE( TypeCheck<MSArray<int>::size_type>::check(
                   cmi2.size()) );
    ...
    SUBCASE( "begin (const) returns ptr-to-item by value" )
    {
        ...
        REQUIRE( TypeCheck<const MSArray<int>::value_type *>::check(
                   cmi2.begin()) );
        ...
        REQUIRE( TypeCheck<const MSArray<double>::value_type *>::check(
                   cmd5.begin()) );
    }
    ...
    SUBCASE( "begin (non-const) returns ptr-to-item by value" )
    {
        ...
        REQUIRE( TypeCheck<MSArray<int>::value_type *>::check(
                   mi1.begin()) );
...
TEST_CASE( "Functions take const arguments (passes if it compiles)" )
{
    SUBCASE( "Taking const MSArray arguments" )
    {
        ...
        [[maybe_unused]]
          bool result1 = (cmi1 == cmi2);  // op==(MSArray,MSArray)
        [[maybe_unused]]
          bool result2 = (cmi1 != cmi2);  // op!=(MSArray,MSArray)
        [[maybe_unused]]
          bool result3 = (cmi1 <  cmi2);  // op<(MSArray,MSArray)
        [[maybe_unused]]
          bool result4 = (cmi1 <= cmi2);  // op<=(MSArray,MSArray)
        [[maybe_unused]]
          bool result5 = (cmi1 >  cmi2);  // op>(MSArray,MSArray)
        [[maybe_unused]]
          bool result6 = (cmi1 >= cmi2);  // op>=(MSArray,MSArray)
...
TEST_CASE( "Comparisons allow min-functionality value types "
...
TEST_CASE( "Checks for noexcept" )
{
    ...
    {
        REQUIRE( noexcept(MSArray<int>(std::move(mi1))) );
    }
...
TEST_CASE( "Bracket operator" )
{
    ...
    SUBCASE( "Change one item; items have correct values (use op[])" )
    {
        ...
        REQUIRE( allgood );
          // All values in array are correct (non-const)
...
TEST_CASE( "Copy ctor" )
{
    ...
    SUBCASE( "Copy ctor - "
               "changing item in original does not affect copy" )
    {
    ...
    SUBCASE( "Copy ctor - "
               "changing item in copy does not affect original" )
    {
...
TEST_CASE( "Same-object tests" )
{
    ...
    SUBCASE( "Move ctor creates object with original array" )
    {
        ...
        MSArray<int> mi2(std::move(mi_movable));
        REQUIRE( &(mi2[0]) == addr1 );
    }
    ...
    SUBCASE( "Move assignment returns *this" )
    {
        MSArray<int> mi_movable(1);
        MSArray<int> mi2;
        MSArray<int> & r(mi2 = std::move(mi_movable));
        REQUIRE( &r == &mi2 );
    }
    ...
    SUBCASE( "end (non-const) "
               "returns pointer to end of original array" )
    {
        MSArray<int> mi(1);
        int * addr1(mi.end());
        int * addr2(mi.end());
   ...
   Counter::reset();               // Restart all counts
    ...
    SUBCASE( "Move ctor: value-type ctor/dctor/assignment counts" )
    {
        {  // Block, so we get dctor calls before function ends
            ...
            const MSArray<Counter> mc2(std::move(mc1));  // Move ctor
    ...
    SUBCASE( "Copy assignment: "
               "value-type ctor/dctor/assignment counts" )
    {
        {  // Block, so we get dctor calls before function ends
            const MSArray<Counter> mc1(thesize);
            MSArray<Counter> mc2;
