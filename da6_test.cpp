...
TEST_CASE( "SLLMap: Member function return Types" )
{
    SUBCASE( "Member function empty - return type")
    {
        {
        const SLLMap<int, string> m;
        bool && r(m.empty());
        (void)r;  // Avoid unused-variable warning
        INFO( "const SLLMap<T,U>::empty returns bool by val" );
        ...
        bool && r(m.empty());
        (void)r;  // Avoid unused-variable warning
        INFO( "non-const SLLMap<T,U>::empty returns bool by val" );
        REQUIRE( TypeCheck<bool>::check(m.empty()) );
        }
    }
    ...
    SUBCASE( "Member function size - return type")
    {
        ...
        size_t && r(m.size());
        (void)r;  // Avoid unused-variable warning
        INFO( "const SLLMap<T,U>::size returns size_t by val" );
        ...
        size_t && r(m.size());
        (void)r;  // Avoid unused-variable warning
        INFO( "non-const SLLMap<T,U>::size returns size_t by val" );
        REQUIRE( TypeCheck<size_t>::check(m.size()) );
        }
    }
    ...
    SUBCASE( "Member function get - return type")
    {
        ...
        SLLMap<int, string> m;
        m.set(1, "abc");
        const SLLMap<int, string> & cm(m);
        const string & r(cm.get(1));
        (void)r;  // Avoid unused-variable warning
        INFO( "const SLLMap<int,string>::get returns string"
              " by ref-to-const" );
        ...
        string & r(m.get(1));
        (void)r;  // Avoid unused-variable warning
        INFO( "non-const SLLMap<int,string>::get returns string"
              " by ref" );
        ...
        SLLMap<int, int> m;
        m.set(1, 2);
        const SLLMap<int, int> & cm(m);
        const int & r(cm.get(1));
        (void)r;  // Avoid unused-variable warning
        INFO( "const SLLMap<int,int>::get returns int"
              " by ref-to-const" );
        ...
        int & r(m.get(1));
        (void)r;  // Avoid unused-variable warning
        INFO( "non-const SLLMap<int,int>::get returns int by ref" );
        REQUIRE( TypeCheck<int &>::check(m.get(1)) );
        }

        {
        SLLMap<string, string> m;
        m.set("xyz", "abc");
        const SLLMap<string, string> & cm(m);
        const string & r(cm.get("xyz"));
        (void)r;  // Avoid unused-variable warning
        INFO( "const SLLMap<string,string>::get returns string"
              " by ref-to-const" );
        ...
        string & r(m.get("xyz"));
        (void)r;  // Avoid unused-variable warning
        INFO( "non-const SLLMap<string,string>::get returns string"
              " by ref" );
...
TEST_CASE( "SLLMap: set, get, erase" )
{
    SUBCASE( "Empty SLLMap" )
    {
        SLLMap<int, string> m;
        const SLLMap<int, string> cm;

        {
            INFO( "empty #1 returns true for empty SLLMap" );
            REQUIRE( m.empty() );
        }
        {
            INFO( "size #1 returns 0 for empty SLLMap" );
            REQUIRE( m.size() == 0 );
        }
        {
            INFO( "present #1 on empty SLLMap returns false" );
            REQUIRE( m.present(1) == false );
        }
        {
            INFO( "present #2 on empty SLLMap returns false" );
            REQUIRE( m.present(100) == false );
        }
        {
            INFO( "empty #2 returns true for empty SLLMap" );
            REQUIRE( m.empty() );
        }
        {
            INFO( "size #2 returns 0 for empty SLLMap" );
            REQUIRE( m.size() == 0 );
        }
        {
            INFO( "get throws out_of_range for empty SLLMap" );
            try
            {
                m.get(1);
                REQUIRE(false);
        ...
        {
            INFO( "get throws out_of_range for empty const SLLMap" );
            try
            {
                cm.get(1);
                REQUIRE(false);
    ...
    SUBCASE( "Size 1 SLLMap" )
    {
        ...
        {
            INFO( "empty returns false for size 1 SLLMap" );
            REQUIRE( !m.empty() );
        }
        {
            INFO( "size returns 1 for size-1 SLLMap" );
            REQUIRE( m.size() == 1 );
        }
        {
            INFO( "present #1 on size-1 SLLMap returns nullptr" );
            REQUIRE( m.present(1) == false );
        }
        {
            INFO( "present #2 on size-1 SLLMap returns true" );
            REQUIRE( m.present(100) == true );
        }
        {
            INFO( "get #1 on size-1 SLLMap throws out_of_range" );
            try
            {
                m.get(1);
                REQUIRE(false);
        ...
        {
            INFO( "get #2 on size-1 SLLMap returns value" );
            auto v = m.get(100);
            REQUIRE(v == "abc");
        }
        {
            INFO( "get #1 on size-1 const SLLMap"
                  "throws out_of_range" );
            try
            {
                cm.get(1);
        ...
        {
            INFO( "get #2 on size-1 const SLLMap returns value" );
            auto v = cm.get(100);
            REQUIRE(v == "abc");
        }
    }

    SUBCASE( "Size 2 SLLMap" )
    {
        SLLMap<int, string> m;
        m.set(100, "abc");
        m.set(101, "def");
        ...
        {
            INFO( "empty returns false for size 2 SLLMap" );
            REQUIRE( !m.empty() );
        }
        {
            INFO( "size returns 2 for size-2 SLLMap" );
            REQUIRE( m.size() == 2 );
        }
        {
            INFO( "present #1 on size-2 SLLMap returns false" );
            REQUIRE( m.present(1) == false );
        }
        {
            INFO( "get #1 on size-2 SLLMap throws out_of_range" );
            try
            {
                m.get(1);
                REQUIRE(false);
        ...
        {
            INFO( "present #2 on size-2 SLLMap returns true" );
            REQUIRE( m.present(100) == true );
        }
        {
            INFO( "get #2 on size-2 SLLMap returns value inserted" );
            REQUIRE( m.get(100) == "abc" );
        }
        {
            INFO( "present #3 on size-2 SLLMap returns true" );
            REQUIRE( m.present(101) == true );
        }
        {
            INFO( "get #3 on size-2 SLLMap returns value inserted" );
            REQUIRE( m.get(101) == "def" );
        }
        {
            INFO( "present #4 on size-2 SLLMap returns false" );
    ...
    SUBCASE( "Modifying value in map" )
    {
        ...
        {
            INFO( "get #1 on size-2 SLLMap returns value inserted" );
            REQUIRE( m.get(100) == "abc" );
        }
        {
            INFO( "get #2 on size-2 SLLMap returns value inserted" );
        ...
        {
            INFO( "get #1 on size-2 SLLMap returns new value" );
            REQUIRE( m.get(100) == "x" );
        }
        {
            INFO( "get #2 on size-2 SLLMap returns original value" );
            REQUIRE( m.get(101) == "def" );
        }
    }
